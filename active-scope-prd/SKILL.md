...
description: Define the active scope with the operator and capture it as a detailed PRD that refines the project PRD without contradicting it — pulling every reference the scope builds against into one place, drawing a hard limit on what it will not do, and expanding each project requirement it claims into detailed, testable functional requirements. The operator names the features they want; three questions settle the boundary, what coverage outside the happy path is aimed for, and what reaches the user. Runs once per scope, after the project PRD and architecture exist. Trigger on "define the active scope", "next scope", "scope prd", "I want these features in the active scope".
---
# Active-scope PRD
...
- **Pull the references.** Everything this scope builds against is named here, once: the project-PRD requirements it refines, the project-PRD sections it draws on, the design references it renders against. Phases 4 and 5 then read what this document points at, instead of re-deriving it from full scope every time.
...
```
docs/
  project/prd.md                    <- full scope; read in full, and the
...
Then pull the references, in the same pass. **This is the only phase that reads full scope whole**, so a reference not pulled here is one Phase 4 re-derives or Phase 5 guesses at:
...
This is where the document earns its place. A parent in full scope is one line written to survive the whole project; this scope needs it as several lines someone can build and test against this week.
...
**The bar is a test.** Read each requirement back and ask what test would fail if the code didn't do it. If you can't name one, the requirement is still at full-scope altitude: expand it, split it, or cut it. Phase 4 has to write that test onto a criterion, and it reads this document rather than asking you.
...
Three places to check before you finish, because they are where excluded work quietly returns: the **half-features the boundary question carved** (the other side of § 1.1), the **full-scope non-functional requirements this scope isn't held to** (§ 6 names the ones it is), and **the coverage § 1.2 deliberately left out**.
...
1.4 References — everything this scope builds against, pulled in step 1, as a
    ...
    searching full scope again. Architecture sections are not listed — the
...
- a parent you couldn't expand past what full scope already says, and why;
