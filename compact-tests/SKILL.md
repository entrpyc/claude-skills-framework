---
name: compact-tests
description: Split the test suite in two — locally only one happy-path test per built functional requirement of docs/project/prd.md runs, one for each requirement marked ✅ or 🔨; everything else moves to a suite that runs on the GitHub pipeline and never on the operator's machine. Nothing is deleted, no source code and no test file changes. Runs only on a green suite, never to reach green, and reports what stopped being checked locally. Read by finalize, which runs it over the scope's tests before wiping the plan, and pulled directly whenever the operator aims it at a slow suite. Trigger on "compact the tests", "the suite is too slow", "trim the test suite", "cut the edge case tests", "leave only happy path tests", "move the slow tests to CI".
---
# Compact tests

Split the suite in two along one line:

- **Local** — one happy-path test per **functional** requirement of `docs/project/prd.md` § 3 *Features* that is marked ✅ or 🔨. Exactly one per requirement, and nothing else. This is what runs while the operator is waiting.
- **CI** — literally everything else, *plus* all of the above. Runs on the GitHub pipeline, and **never on the operator's machine.**

Nothing is deleted and no test is rewritten. What changes is **where** a test runs, and therefore **when** a failure is found.

The local suite exists to answer one question while someone waits on it: *does the product still do what it is required to do?* One test per requirement answers it. Every test that answers a different question — a boundary, an error branch, a second case of a behavior already covered, a second acceptance criterion of a requirement a sibling test already proves — was being paid for on every local run, and answers it on the pipeline for free.

> **Read the `conventions` skill before anything below.**

**`finalize` runs this over every scope, at its step 4** — while the scope is green and `docs/scope/plan.md` still exists, because that plan is the shortest route from a test to the requirement it proves. Pulled on its own it works the same way, on whatever the operator aims it at.
...
2. **Trace every test to a project requirement.** For each test, answer one question: **which functional requirement in `docs/project/prd.md` § 3 *Features*, marked ✅ or 🔨, does this test prove?**

   The acceptance criterion is a means of answering that, never the unit being counted. Where `docs/scope/plan.md` still exists the chain the system already maintains is the shortest route (`conventions` § Citing): the test is named by an acceptance criterion, the criterion references a scope PRD requirement, and that requirement names its project PRD parent. Where the plan is gone — `finalize` wipes it — trace from the code the test exercises to the requirement that code implements, and the answer is the same one.

   A requirement still marked 📝 has no code, and so nothing traced to it stays local — a test that lands there proves something nothing is yet required to do.
   ...
   - **A non-functional parent** — it lands on `docs/project/prd.md` § 6 *Non-functional requirements* rather than § 3 *Features*. **Name these separately when you ask.** A performance, security or accessibility guarantee that only runs on the pipeline is found broken after the push rather than before it, and that is a different decision from deferring a boundary case.
3. **Pick the one test that stays local, per requirement.** Group what is left by the **functional requirement** it proves, not by the criterion that named it — so every test under a requirement competes with every other one, across criteria. **Exactly one stays local per requirement.**
   ...
   - Where none is a happy path, keep the cheapest and **say the requirement has no happy-path proof.** That is a hole in the tests, not a result of this skill, and it was there before.
   - For a **🔨** requirement, the local test covers the happy path of **the part that is built.** Where what is built has no happy-path test, keep the cheapest test there is and **say so.**
   - **A ✅ or 🔨 functional requirement never drops to zero local tests.** If following the rules would empty one, the rules stop. This is the rail that pins the size of the local suite: as many tests as there are built functional requirements, no more and no fewer.
4. **Put the split to the operator** — `AskUserQuestion`, ordered by seconds saved, **grouped and never enumerated.** Each group says what it is, how many tests, how many seconds it gives back locally, and **what is no longer caught before a push.** This line moves whole acceptance criteria and not only extra cases — a criterion whose requirement a sibling test already proves keeps nothing local of its own — so also say **how many acceptance criteria are left with no local proof at all**, not only how many tests moved. Say what the local suite drops to in total. Whatever they exclude stays local.
...
- **One local test per built functional requirement; every test on the pipeline.**
- **No source code changes, and no test file changes.**
- **A ✅ or 🔨 functional requirement never drops to zero local tests**, whatever it costs in seconds. That rail, and nothing else, sets how large the local suite is.
...
- **Say what stopped being caught before a push**, grouped, one line each, **and how many acceptance criteria no longer have any local proof.** The operator is trading feedback latency for local speed and has to see both halves.
...
- **what is no longer caught locally**, grouped, one line each, **and how many acceptance criteria are left with no local proof**;
