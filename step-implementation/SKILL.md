...
4. **Kill false positives.** Once tests pass, check them against the acceptance criteria for false positives — tests that pass without actually proving the criterion — and fix them.

5. **Output the manual-validation checklist** (exempt from compression — make it complete and specific). It covers one check per acceptance criterion with a note on what to look for, plus the step doc's user steps. This checklist **relays what planning already established — it doesn't invent criteria.** Any work required from the operator is written as clear specific steps.
...
When every acceptance criterion is implemented, the full test suite is green, and false positives are cleared, present the manual-validation checklist and wait for the operator to run it. If tests are still failing, that is what you report — not a finished step. The step closes only when the manual checks pass — then move to the validation prompt.
