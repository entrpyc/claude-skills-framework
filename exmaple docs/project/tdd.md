...
recommendations and note anchors are all the same thing viewed differently — a `(recording, start_ms, end_ms)` tuple with text and an embedding attached. That is why there is one vector index
...
**Cross-references** are a derived edge table over segment pairs — `(segment_a, segment_b, score, basis)` where basis is embedding similarity, shared tag, or shared scripture citation
...
particular ([3.10.9](docs/project/prd.md#L220)) issues one query whose visibility predicate is `published OR owner = :me`, which is only possible because vectors and ownership live in the same database.
