...
| `compact-tests` | Leaves one happy-path test per criterion running locally and moves the rest onto the GitHub pipeline | the project PRD, the plan, the test suite and its timings | the test config, the moved test files and the workflow; no source code, and nothing deleted | at every `finalize`, and whenever the operator pulls it |

Five skills sit alongside the five phases, and none of them delivers a scope: **`conventions`** — the rules every phase follows, read before any of them run — **`dev-system`**, this skill, the map over the whole thing, **`session-analyze`**, which explains where one run's time went and what ordered it, **`refine-dev-system`**, which turns many past runs into edits to the skills, and **`compact-tests`**, which moves everything but the happy path off the operator's machine and onto the pipeline, once they have chosen what moves — pulled on its own, and run by `finalize` over every scope before the plan is wiped. The last two are the only ones that change anything — the skills and the test config respectively, never the product.
