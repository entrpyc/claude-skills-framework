...
5. **Reach green** — `conventions` § Reaching green. **Every criterion's test is written before its code, and first run once that code is there.** There is no run before it: a red nobody acts on costs a whole run, and on anything needing a server or a browser it costs several.
   ...
   A criterion is proved by the **narrowest run that covers it**, inside the **local** suite — the pipeline half is never invoked here, whatever it would cover (`conventions` § Reaching green). The whole local suite runs **once**, when the step's **last** substep lands: that is the first time the substeps are measured together, and it is not how an individual criterion is checked. **A target that ends mid-step does not trigger it**, and a red it surfaces in code this target never touched is a pre-existing red — reported in a line, not triaged.

   When a red's output does not name its cause, instrument it under `conventions` § Probing a failure rather than guessing a fix and paying a run per guess.
